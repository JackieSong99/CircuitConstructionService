...
class CircuitConstructionViewSet(viewsets.ModelViewSet):
    ...
    def construct(self, request):
        # curl -X POST http://127.0.0.1:8000/api/circuit-construction/construct/ -H 'Content_Type: application/json' -d
        # '@circuit_input_test6.json'
        print("------------------------------------------")
        print(simplejson.loads(request.body))
        print("------------------------------------------")

        with open('circuit_json_data.json', 'w',  encoding='utf-8') as f:
            json.dump(simplejson.loads(request.body), f, ensure_ascii=False,  indent=4)
    ...
    def test(self, request):
        # curl -X POST http://127.0.0.1:8000/api/circuit-construction/test/ -H 'Content_Type: application/json' -d
        # '@circuit_input_test6.json'
        print("**************************************")
